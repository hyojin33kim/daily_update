...
const dailyOptions={responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>items[0].label}}},scales:{x:{ticks:{autoSkip:false,maxRotation:50,minRotation:50,font:{size:8}}}}};
const vixDaily=D.vix.daily.slice(-10),vixCanvas=document.getElementById('vixChart');vixCanvas.parentElement.style.height='150px';const vixOld=Chart.getChart('vixChart');if(vixOld)vixOld.destroy();new Chart(vixCanvas,{type:'line',data:{labels:vixDaily.map(x=>x.date.slice(5).replace('-','/')),datasets:[{data:vixDaily.map(x=>x.value),borderColor:'#0078d4',pointBackgroundColor:'#0078d4',pointRadius:4,tension:.2}]},options:dailyOptions});
const pcDaily=D.put_call.daily.slice(-10),pcCanvas=document.getElementById('pcChart');pcCanvas.parentElement.style.height='150px';const pcOld=Chart.getChart('pcChart');if(pcOld)pcOld.destroy();new Chart(pcCanvas,{type:'line',data:{labels:pcDaily.map(x=>x.date.slice(5).replace('-','/')),datasets:[{data:pcDaily.map(x=>x.value),borderColor:'#f97316',pointBackgroundColor:'#f97316',pointRadius:4,tension:.2}]},options:dailyOptions});
if(s[1])s[1].querySelector('.sent-title').textContent='② VIX 변동성 · 최근 10거래일';
if(s[2])s[2].querySelector('.sent-title').textContent='③ Equity Put/Call · 최근 10거래일';
