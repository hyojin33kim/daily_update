...
kpi('VIX 변동성',fmt(D.vix.value,2),`${D.vix.as_of} 종가`);kpi('P/C Ratio','N/A',D.put_call.note);
...
const s=document.querySelectorAll('.sent-card');if(s[0])s[0].querySelector('.sent-desc').innerHTML=`CNN F&amp;G <strong>${fmt(D.fear_greed.value,1)}/100</strong> · ${D.fear_greed.as_of.slice(0,10)}`;if(s[1])s[1].querySelector('.sent-desc').innerHTML=`현재 <strong>${fmt(D.vix.value,2)}</strong> · ${D.vix.as_of} 종가`;if(s[2])s[2].querySelector('.sent-desc').textContent=D.put_call.note;if(s[3])s[3].querySelector('.sent-desc').textContent=D.aaii.note;
...
const f=document.querySelector('.footer');if(f)f.innerHTML=`기준일 ${k.as_of} · <a href="${k.source}">KOSPI</a> · <a href="${D.vix.source}">VIX</a> · <a href="${D.fear_greed.source}">CNN Fear &amp; Greed</a> · N/A는 검증 가능한 무료 최신 자료가 없는 항목입니다.`;
...
const vixOld=Chart.getChart('vixChart');if(vixOld)vixOld.destroy();new Chart(document.getElementById('vixChart'),{type:'line',data:{labels:D.vix.monthly.map(x=>x.date),datasets:[{data:D.vix.monthly.map(x=>x.value),borderColor:'#0078d4',pointRadius:3,tension:.25}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
['pcChart','aaiiChart'].forEach(id=>{const old=Chart.getChart(id);if(old)old.destroy();});
})();
