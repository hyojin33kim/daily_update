...
const dailyOptions={responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>items[0].label}}},scales:{x:{ticks:{autoSkip:true,maxTicksLimit:9,maxRotation:45,minRotation:0,font:{size:8}}}}};
const fgDaily=D.fear_greed.daily,fgCanvas=document.getElementById('fgChart');const fgOld=Chart.getChart('fgChart');if(fgOld)fgOld.destroy();new Chart(fgCanvas,{type:'line',data:{labels:fgDaily.map(x=>x.date),datasets:[{data:fgDaily.map(x=>x.value),borderColor:'#dc2626',backgroundColor:'rgba(220,38,38,.08)',pointBackgroundColor:'#dc2626',pointRadius:2,fill:true,tension:.15}]},options:{...dailyOptions,scales:{...dailyOptions.scales,y:{min:0,max:100}}}});
const vixDaily=D.vix.daily,vixCanvas=document.getElementById('vixChart');vixCanvas.parentElement.style.height='170px';const vixOld=Chart.getChart('vixChart');if(vixOld)vixOld.destroy();new Chart(vixCanvas,{type:'line',data:{labels:vixDaily.map(x=>x.date),datasets:[{data:vixDaily.map(x=>x.value),borderColor:'#0078d4',pointBackgroundColor:'#0078d4',pointRadius:2,tension:.15}]},options:dailyOptions});
const pcDaily=D.put_call.daily,pcCanvas=document.getElementById('pcChart');pcCanvas.parentElement.style.height='170px';const pcOld=Chart.getChart('pcChart');if(pcOld)pcOld.destroy();new Chart(pcCanvas,{type:'line',data:{labels:pcDaily.map(x=>x.date),datasets:[{data:pcDaily.map(x=>x.value),borderColor:'#f97316',pointBackgroundColor:'#f97316',pointRadius:2,tension:.15}]},options:dailyOptions});
if(s[1])s[1].querySelector('.sent-title').textContent='② VIX 변동성 · 최근 6개월 일별';
if(s[2])s[2].querySelector('.sent-title').textContent='③ Equity Put/Call · 최근 6개월 일별';
if(s[0])s[0].querySelector('.sent-title').textContent='① Fear & Greed · 최근 6개월 일별';
